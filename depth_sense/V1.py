...
if __name__ == '__main__':
    ...
    parser.add_argument('--encoder', type=str, default='vits', choices=['vits', 'vitb', 'vitl', 'vitg'], help='Model encoder type')  # Changed default to 'vits'
    parser.add_argument('--grayscale', dest='grayscale', action='store_true', help='Display depth map in grayscale')
    ...
    try:
        while True:
            ...
            # Display depth map in grayscale or color
            if args.grayscale:
                depth_display = np.repeat(depth[..., np.newaxis], 3, axis=-1)
            else:
                cmap = cv2.COLORMAP_JET
                depth_display = cv2.applyColorMap(depth, cmap)

            # Show the depth map
            cv2.imshow("Live Depth Map", depth_display)
